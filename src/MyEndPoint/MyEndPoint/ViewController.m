...
//  MyEndPoint
//
//  Created by Zimry Ong on 20.11.2020.
//
...
- (IBAction)Install:(id __unused)sender {
    if (self.currentRequest) {
        NSBeep();
        return;
    }
    NSLog(@"Install the extension");

    OSSystemExtensionRequest *req = [OSSystemExtensionRequest activationRequestForExtension:@"com.test.MyEndPoint.Extension" queue:dispatch_get_main_queue()];
    req.delegate = (id<OSSystemExtensionRequestDelegate>)self;

    [[OSSystemExtensionManager sharedManager] submitRequest:req];
    self.currentRequest = req;
    [self logWithFormat:@"Installing the extension 🔄\n"];
